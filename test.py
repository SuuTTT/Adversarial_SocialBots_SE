...
def test(
    ...
    def get_action(obs, agent=None, env=None, greedy=None, page_rank=None):
        action = {}
        if not greedy:
            explores = {
                'level1': False,
            ...
            for agent_id, agent_obs in obs.items():
                if agent_id == "level1":
                    ...
                    if use_pagerank:
                        # Call the next_best_pagerank function for level2 agent
                        action[agent_id] = env.next_best_pagerank()
                    else:
