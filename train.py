...
def train(
    ...
    checkpoint_freq=5,
    wandb_key=None,
    node_percentage=0.1):  # New parameter

    def env_creator(_,  **kwargs):
        env = AdvBotEnvSingleDetectLargeHiar(seed=seed,
                                            ...
                                            node_percentage=kwargs.get("node_percentage", 0.1))  # Pass node_percentage
        return env
    ...
    config={
        ...
        "env": NAME,
        "env_config": {
            "node_percentage": tune.grid_search([1, 0.96, 0.98,0.99])  # Replace with the list of percentages you want to try
            #"node_percentage": tune.grid_search([1, 0.96, 0.98,0.99])
        }
